...
l1 = ['E0','B1','SP1','D1','N1','I1','P1','F1','T1','SC0']
l2 = ['E1','SP2','D2','F2','SC1']
...
def build_match_features(chpt, hometeam, awayteam, short=4):
    ...
    db = pd.read_csv(dict_keys[key])[columns_req]
...
def build_database(chpt, short, train_ratio):
    ...
    db_train.to_csv(os.path.join(path, "models/"+ chpt+"_train_dataset.csv"),index=None)
    db_test.to_csv(os.path.join(path, "models/"+ chpt+"_test_dataset.csv"),index=None)
    print(db_train)
    print(db_test)
# %%
def build_all_database(ll, short, train_ratio):
    ...
    for chpt in ll:#+l2:
        ...
        dftrain.append(pd.read_csv(os.path.join(path, "models/"+ chpt+"_train_dataset.csv")))
        dftest.append(pd.read_csv(os.path.join(path, "models/"+ chpt+"_test_dataset.csv")))
    db_train=pd.concat(dftrain, ignore_index=True)
    db_train.to_csv(os.path.join(path, "models/all_train_dataset.csv"),index=None)
    db_test=pd.concat(dftest, ignore_index=True)
    db_test.to_csv(os.path.join(path, "models/all_test_dataset.csv"),index=None)

# %%
def learn_model(chpt, epoch=100):
    path = os.path.dirname(os.path.abspath(__file__))
    ...
    train_data=pd.read_csv(os.path.join(path,"models/"+chpt+"_train_dataset.csv"))
    test_data=pd.read_csv(os.path.join(path,"models/"+chpt+"_test_dataset.csv"))
    ...
    # %%
    model.fit(train_input, train_output, epochs=epoch)
    model.summary()
    model.save(os.path.join(path,"models/"+chpt+"_model.h5"))
